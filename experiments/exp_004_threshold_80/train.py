#!/usr/bin/env python3
"""
exp_004: 깡통전세 분류기 — 위험 임계값 80% (LightGBM + SHAP)

exp_003 대비 변경점:
- 위험 라벨 임계값: 90% → 80% (전세가율 >= 0.80 → 위험)
- 안전 임계값 유지: <= 0.60
- 미분류 구간 축소: 60~90% → 60~80%
- 더 많은 '위험' 샘플 확보 → 모델 학습 데이터 증가 기대
"""
...
def train_and_evaluate(features, labels):
    ...
    best_model.save_model(str(MODELS_DIR / "lgbm_gangton_v4_t80.txt"))
...
def main():
    print("=" * 60)
    print("exp_004: 깡통전세 분류기 — 위험 임계값 80% (LightGBM)")
    print("=" * 60)
    ...
    train_log = {
        "experiment_id": "exp_004_threshold_80",
        "model_type": "gangton_classifier",
        "hypothesis": "위험 임계값을 90%→80%로 낮추면 미분류 구간(60~80%)이 줄고 위험 샘플이 증가하여 모델이 경계 패턴을 더 잘 학습할 수 있는지 검증",
        "base_experiment": "exp_003_building_features",
        "changes": [
            "danger threshold: 0.90 -> 0.80",
            "unlabeled zone: 60-90% -> 60-80%",
            "safe threshold unchanged: 0.60",
        ],
    ...
    print(f"  모델: {MODELS_DIR / 'lgbm_gangton_v4_t80.txt'}")
