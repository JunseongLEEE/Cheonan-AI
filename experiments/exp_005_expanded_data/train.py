#!/usr/bin/env python3
"""
exp_005: 깡통전세 분류기 — 확장된 8유형 데이터셋 재학습 (LightGBM + SHAP)

exp_004 대비 변경점:
- 데이터셋 확장: 8개 부동산 유형 포함 (Rent 371,612 / Trade 270,307 / Jeonse 188,184)
- 모델/피처/하이퍼파라미터/임계값은 exp_004와 동일
- 위험 임계값: >= 0.80, 안전 임계값: <= 0.60
"""
...
def compute_dong_historical_risk(df):
    ...
    df["위험여부_tmp"] = (df["전세가율"] >= 0.80).astype(int)
...
def train_and_evaluate(features, labels):
    ...
    best_model.save_model(str(MODELS_DIR / "lgbm_gangton_v5_expanded.txt"))
...
def main():
    print("=" * 60)
    print("exp_005: 깡통전세 분류기 — 확장 8유형 데이터셋 (LightGBM)")
    print("=" * 60)
    ...
    train_log = {
        "experiment_id": "exp_005_expanded_data",
        "model_type": "gangton_classifier",
        "hypothesis": "8개 부동산 유형으로 확장된 데이터셋(Rent 371K, Trade 270K, Jeonse 188K)으로 재학습하면 더 다양한 패턴을 학습하여 성능이 유지 또는 개선되는지 검증",
        "base_experiment": "exp_004_threshold_80",
        "changes": [
            "expanded dataset: 8 property types included",
            "Rent 371,612 / Trade 270,307 / Jeonse 188,184 rows",
            "same features, thresholds, hyperparams as exp_004",
        ],
    ...
    print(f"  모델: {MODELS_DIR / 'lgbm_gangton_v5_expanded.txt'}")
